...
export interface CanvasObjectSpec {
  ...
  kind: 'rect' | 'circle' | 'textbox' | 'line' | 'path' | 'polygon' | 'image' | 'group' | 'latex_svg' | 'graph_layout' | string; // Common types + allow custom, added new kinds
  x?: number | string; // Changed to allow string for percentage values
  y?: number | string; // Changed to allow string for percentage values
  xPct?: number; // Optional: 0.0 to 1.0
  yPct?: number; // Optional: 0.0 to 1.0
  fill?: string;
  stroke?: string;
  strokeWidth?: number;
  width?: number | string; // Changed to allow string for percentage values
  height?: number | string; // Changed to allow string for percentage values
  widthPct?: number; // Optional: 0.0 to 1.0
  heightPct?: number; // Optional: 0.0 to 1.0
  radius?: number; // For circle
  text?: string; // For textbox
  fontSize?: number; // For textbox
  fontFamily?: string; // For textbox
  points?: number[] | {x: number, y: number}[] | string; // For line, path, polygon. String for SVG paths
  angle?: number;
  selectable?: boolean;
  evented?: boolean; // Typically false for AI drawings?
  src?: string; // For image
  objects?: CanvasObjectSpec[]; // For group spec
  size?: number; // For radio/checkbox etc. - Consider if this is still needed or should be part of metadata
  metadata?: { // Add metadata field
    id: string; // Ensure ID is always in metadata too
    source?: 'assistant' | 'user' | string;
    groupId?: string; // For group membership tracking
    pctCoords?: { xPct?: number; yPct?: number; widthPct?: number; heightPct?: number }; // Store original % if used
    latex?: string; // For latex_svg kind
    layoutSpec?: any; // For graph_layout kind
...
export type WhiteboardAction =
  ...
  | { type: "UPDATE_OBJECTS"; objects: Partial<CanvasObjectSpec>[] } // MUST include 'id' in each partial object
  | { type: "UPDATE_OBJECT"; objectId: string; updates: Partial<CanvasObjectSpec> }
  | { type: "DELETE_OBJECTS"; ids?: string[]; objectIds?: string[] }
  | { type: "CLEAR_CANVAS"; scope?: 'all' | 'assistant_only' | 'visual_only' } // Add scope, added 'visual_only'
  | { type: 'GROUP_OBJECTS'; groupId: string; objectIds: string[] }
  | { type: 'MOVE_GROUP'; groupId: string; dx: number; dy: number }
  | { type: 'DELETE_GROUP'; groupId: string }
  | { type: 'ADD_EPHEMERAL'; spec: CanvasObjectSpec }
  | { type: 'DELETE_EPHEMERAL'; id: string };
