...
export const processEmbeddingQueue = internalAction({
  ...
  handler: async (ctx) => {
    try {
      const result = await ctx.runAction(internal.documentProcessor.processEmbeddingQueue, {
        batchSize: 5,
      });
...
export const batchProcessSessionAnalytics = internalAction({
  ...
  handler: async (ctx) => {
    try {
      const result = await ctx.runAction(internal.tutorEndpoints.processSessionAnalytics, {
        batchSize: 50,
      });
...
export const processEmbeddingJob = action({
  ...
  handler: async (ctx, args) => {
    const job = await ctx.runQuery(internal.backgroundJobs.getJobStatus, {
      jobId: args.jobId,
    });
    ...
    try {
      // Mark job as running
      await ctx.runMutation(internal.backgroundJobs.updateJobProgress, {
        jobId: args.jobId,
        status: "running",
        progress: 0,
      });
      ...
      await ctx.runMutation(internal.documentProcessor.updateEmbeddingStatus, {
...
export const batchProcessSessionAnalytics = action({
  ...
  handler: async (ctx, args) => {
    ...
    for (const session of sessionsNeedingAnalytics) {
      try {
        const analytics = await ctx.runAction(internal.tutorEndpoints.processSessionAnalytics, {
