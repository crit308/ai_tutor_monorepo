    ...
    try {
      ...
      const streamData = async () => {
        ...
        try {
          ...
          const userMessageResult = await ctx.runMutation(internal.functions.addSessionMessage, {
            sessionId: sessionId as Id<'sessions'>,
            role: 'user',
            text: message,
          ...
          const session = await ctx.runQuery(internal.functions.getSessionEnhanced, { 
            sessionId: sessionId as Id<'sessions'> 
          });
          ...
          const plannerResult = await ctx.runAction(internal["agents/actions"].planSessionFocus, {
            sessionId,
            userId,
          ...
          const aiMessageResult = await ctx.runMutation(internal.functions.addSessionMessage, {
            sessionId: sessionId as Id<'sessions'>,
            role: 'assistant',
            text: "",
          ...
          const systemPrompt = plannerResult.success && plannerResult.data?.approach 
            ? plannerResult.data.approach 
          ...
          await ctx.runMutation(internal.functions.updateSessionMessage, {
